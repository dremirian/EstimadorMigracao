...
export function PDFGenerator({ config, databases, activities, diskSegregation, databaseList }: PDFGeneratorProps) {
  const generatePDF = async () => {
    ...
    const addText = (text: string, x: number, y: number, maxWidth?: number, fontSize = 10, color: number[] = [0, 0, 0]) => {
    ...
    doc.text(`Cliente: ${config.clientName}`, pageWidth / 2, 30, { align: 'center' });
    
    yPosition = 50;

    // Configuration section with colored header
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    yPosition = addText(diskSegregation, margin + 5, yPosition, pageWidth - 2 * margin - 10, 9);
    yPosition += 10;
  ...
  yPosition += 20;
    ...
    if (activities.length > 0) {
      ...
      activities.forEach((activity, index) => {
        ...
        // Activity number in colored circle
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(0, 0, 0);
        yPosition = addText(activity.description, margin + 12, yPosition + 4, pageWidth - margin - 80, 8);
    ...
    // Save the PDF
    const fileName = `relatorio-migracao-${config.clientName.replace(/\s+/g, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };
